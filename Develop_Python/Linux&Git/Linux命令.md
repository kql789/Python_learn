...
## raid0,raid1,raid5 
raid0  
1. 特点：数据分散存储在多个硬盘,每个硬盘都负责一部分数据。
2. 优点：读写并行，速度超快，提升数倍。
3. 缺点：一但一个硬盘挂掉，则损坏全部数据，因为没有冗余备份。。
4. 应用：RAID 0更适合用于非关键数据或需要临时高性能的应用，而不适用于关键数据的长期存储。

raid1  
1. 特点：数据分别（写了两份）同时存储在两个或更多硬盘上。
2. 优点：实现了数据备份，提供了数据冗余，但不提高性能。
3. 缺点：磁盘使用率只能到50%，如果一个硬盘故障，数据仍然可以从另一个硬盘恢复。
4. 应用：RAID 1通常用于关键数据的备份和冗余。

raid5  
1. 特点：提供热备盘实现故障恢复，数据被分布在多个硬盘上，同时每个硬盘上也存储了奇偶校验数据。
2. 优点：只损坏1块磁盘，数据不会损坏，提供了一定的冗余和性能提升。
3. 缺点：如果一个硬盘故障，数据可以通过奇偶校验数据来重建，而不需要完全的镜像。同时损坏2块磁盘，数据损坏。
4. 应用：RAID 5通常用于中小型企业中，以提供一定的性能和冗余。
